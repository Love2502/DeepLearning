...
Sampling & full official final run, demo subsets only in Streamlit \\
...
Dataset use & Official trainval/test split \\
Final evaluation & Full official test split \\
\bottomrule
\end{tabular}
\end{center}
\vspace{0.3cm}
...
Breed & ResNet18 transfer & 0.857 & 0.856 \\
Species & Scratch CNN & 0.722 & 0.635 \\
Species & ResNet18 transfer & 0.983 & 0.980 \\
\bottomrule
\end{tabular}
\end{center}
\vspace{0.2cm}
...
\begin{frame}{Breed Prediction Examples}
\IfFileExists{../outputs/figures/breed_transfer_prediction_grid.png}{
\centering
\includegraphics[height=0.82\textheight,width=0.96\linewidth,keepaspectratio]{../outputs/figures/breed_transfer_prediction_grid.png}
}{}
\end{frame}
...
\textbf{Limitations}
\begin{itemize}
    \item Training time is still limited for a class project.
    \item The scratch CNN may need more epochs for 37 breeds.
    \item Transfer learning has an advantage from ImageNet features.
\end{itemize}
\column{0.48\textwidth}
\textbf{Next steps}
\begin{itemize}
    \item Try longer training and larger image sizes.
...
\vspace{0.3cm}
\begin{itemize}
    \item Shared setup: virtual environment, notebook run, and \texttt{python main.py}.
